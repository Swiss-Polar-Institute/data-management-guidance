...
$endif$
\date{$date$ \\ {\includegraphics[scale=0.5]{by.png}} \\ DOI: \href{https://doi.org/$doi$}{$doi$} }
$if(beamer)$
$if(titlegraphic)$
...
\fancyfoot[L]{\fontsize{10}{11}\selectfont \date{$date$} \\ {$version$}}
...
{
  % \fancyhead[C]{\parbox[b]{4.5cm}{Phone: +41 21 693 76 74 \\ Phone: +41 21 693 10 74 \\ Web: www.swisspolar.ch}}
  \fancyhead[L]{\includegraphics[scale=0.6]{spi_logo_hd.png}}
  \fancyfoot[C]{\includegraphics[scale=0.15]{spi_contact_details.png}}
  \fancyfoot[L]{\includegraphics[scale=0.15]{compass.png}}
  % \fancyfoot[R]{DOI \href{https://doi.org/$doi$}{$doi$}}
...
\begin{document}
\frenchspacing % Keeps the space after a full stop smaller (specially when it's left-right justified)
$if(title)$
$if(beamer)$
\frame{\titlepage}
$else$
